...
import { createGlobalTemplate, applyGlobalTemplatesToAllTenants } from "../actions";
import type {
  TemplateType,
  TemplateSchema,
  ReportVisibility,
...
export function NewGlobalTemplateForm() {
  ...
  const [targetRoles, setTargetRoles] = useState<string[]>(["member", "manager"]);
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        // Auto-apply to all tenants if checked
        if (autoApply) {
          await applyGlobalTemplatesToAllTenants();
        }
        router.push("/admin/global-templates");
      } else {
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <TemplateBuilder
            ...
            templateName={name || "新規グローバルテンプレート"}
            templateType={type}
            onSave={setSchema}
          />
        </CardContent>
